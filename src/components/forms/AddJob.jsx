...
export default function AddJob() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setValue("salaryRange", { min: "", max: "" });
      setValue("jobDescription", "");
      setValue("keyResponsibilities", []);
      setValue("requiredSkillsAndQualifications", []);
      setValue("benefits", []);
      setValue("applicationDeadline", "");
      setTimeout(() => router.push("/home/jobs"), 2000);
    } catch (err) {
      setError(err.response?.data?.message || "Error creating job post");
    } finally {
      setLoading(false);
    }
  };
